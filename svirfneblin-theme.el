;;; svirfneblin-theme.el --- Dark theme inspired by svirfneblin

;; Copyright (C) 2013-2016  Simon Manning
;; Copyright © 2018 Jeff Tecca

;; Author: Jeff Tecca <jefftecca@protonmail.com>
;; Maintainer: Jeff Tecca <jefftecca@protonmail.com>
...
;; A dark color theme for Emacs 24+ (using deftheme), inspired by
;; svirfneblin-theme by Simon Manning but with slightly tweaked colors to fit in
;; more with Adwaita Dark for Gnome 3.
...
(defconst svirfneblin-description
  "A Light Table inspired color theme based on Solarized's definitions.")

(defcustom svirfneblin-degrade nil
  "For test purposes only; when in GUI mode, forces Svirfneblin to use the 256
degraded color mode to test the approximate color values for accuracy."
  :type 'boolean
  :group 'lt)

(defcustom svirfneblin-diff-mode 'normal
  "Sets the level of highlighting to use in diff-like modes."
  :type 'symbol
  :options '(high normal low)
  :group 'lt)

(defcustom svirfneblin-bold t
  "Stops Svirfneblin from displaying bold when nil."
  :type 'boolean
  :group 'lt)

(defcustom svirfneblin-underline t
  "Stops Svirfneblin from displaying underlines when nil."
  :type 'boolean
  :group 'lt)

(defcustom svirfneblin-italic t
  "Stops Svirfneblin from displaying italics when nil."
  :type 'boolean
  :group 'lt)

(defcustom svirfneblin-contrast 'normal
  "Stick with normal! It's been carefully tested. Setting this option to high or
low does use the same Svirfneblin palette but simply shifts some values up or
...
the \"Gen RGB\" column in svirfneblin-definitions.el to improve them further."
  :type 'boolean
  :group 'lt)
...
(defvar svirfneblin-colors           ; ANSI(Svirfneblin terminal)
  ...
  '((base04  "#1a1d1e" "#1a1d1e" "#1a1d1e" "brightblack"   "black")
    ...
    (base0   "#eeeeec" "#eeeeec" "#eeeeec" "brightblue"    "blue")
  ...
  "This is a table of all the colors used by the Svirfneblin color theme. Each
...
(defun svirfneblin-color-definitions (mode)
  (noct-flet ((find-color (name)
                          (let* ((index (if window-system
                                            (if svirfneblin-degrade
                                                3
                                              (if svirfneblin-broken-srgb 2 1))
                                          (case (display-color-cells)
                                            (16 4)
                                            (8  5)
                                            (otherwise 3)))))
                            (nth index (assoc name svirfneblin-colors)))))
             (let ((base03      (find-color 'base03))
                   ...
                   (bold        (if svirfneblin-bold 'bold 'normal))
                   (bright-bold (if svirfneblin-bold 'normal 'bold))
                   (underline   (if svirfneblin-underline t nil))
                   (opt-under   nil)
                   (italic      (if svirfneblin-italic 'italic 'normal)))
               (let ((back base03))
                 (cond ((< (display-color-cells) 16)
                        (setf back nil))
                       ((eq 'high svirfneblin-contrast)
                        (let ((orig-base3 base3))
                          (rotatef base01 base00 base0 base1 base2 base3)
                          (setf base3 orig-base3)))
                       ((eq 'low svirfneblin-contrast)
                        (setf back      base02
                              opt-under t)))
                 ...
                 (let ((bg-back   `(:background ,back))
                   ...
                   `((;; basic
                      ...
                      (erc-nick-defausvirfneblin-face ((t (,@fmt-none ,@fg-cyan))))
...
(defmacro create-svirfneblin-theme ()
  (let* ((theme-name 'svirfneblin)
         (defs (svirfneblin-color-definitions 'dark))
         (theme-vars (mapcar (lambda (def) (list (car def) (cdr def)))
                             (second defs)))
         (theme-faces (first defs)))
    `(progn
       (deftheme ,theme-name ,svirfneblin-description)
...
(create-svirfneblin-theme)

(provide-theme 'svirfneblin)

;;; svirfneblin-theme.el ends here
